...
class Keyboard_TerrainCreation(BaseTask):
    ...
    async def set_up_scene(self, scene) -> None:
        ...
        cam = get_prim_at_path('/World/CameraPos')
        reset_and_set_xform_ops(cam, translation = (-4, -(self.dictionary[1]['terrain1']['length'] * self.dictionary[1]['terrain1']['scale'][1])/2, 1.5), orientation = Gf.Quatd(1,0.0,0.0,0.0)) 
    ...
    def get_terrain(self, type, position, orientation, name, width, length, counter, scale, material):
        ...
        add_terrain_to_stage(stage=self._stage, vertices=vertices, triangles=triangles, position=position, orientation=orientation, name="terrain/"+name+str(counter+1), scale=scale, material = material)
        
        
      
    
    async def save_terrain(self):

        input_asset_path = "/home/yohan-sl-intern/devel/Elevation_Map_Testing/isaac_dataset_generation/data/scene.usd"
        output_asset_path = "/home/yohan-sl-intern/devel/Elevation_Map_Testing/isaac_dataset_generation/data/scene.obj"

        self._stage.RemovePrim('/World/CameraPos')
        self._stage.RemovePrim('/Environment')
        self._stage.RemovePrim('/World/PaintTool')
        self._stage.RemovePrim('/World/ActionGraph')

        self._stage.Export(input_asset_path)
        ...
        task = task_manager.create_converter_task(input_asset_path, output_asset_path, self.progress_callback, context)
        ...
        print("The scene has been successfully saved as an obj file")
